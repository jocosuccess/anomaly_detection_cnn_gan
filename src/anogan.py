...
def load_model():
    ...
    d.load_weights('./weights/discriminator_127.h5')
    g.load_weights('./weights/generator_127.h5')
    return g, d
    ...
    for epoch in range(200):
        ...
        for index in range(n_iter):
            ...
            if index % 100 == 0:
                ...
                image = image * 255
                cv2.imwrite('./result/' + str(epoch) + "_" + str(index) + ".png", image[:, :, ::-1])
        ...
        g.save_weights('weights/generator_' + str(epoch) + '.h5', True)
        d.save_weights('weights/discriminator_' + str(epoch) + '.h5', True)
...
### anomaly detection
def compute_anomaly_score(model, x, intermidiate_model, iterations=500):
